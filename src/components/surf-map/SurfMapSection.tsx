...
import { MapPin, Waves, Search, X, SlidersHorizontal, Coffee, UtensilsCrossed, Building2, Fuel, ShoppingBag, Car, Bed } from 'lucide-react';
...
export default function SurfMapSection() {
  ...
  const visibleSpotIds = useMemo(() => {
    if (viewMode === 'amenities') {
      if (selectedAmenity) return new Set([selectedAmenity.spotId]);
      return new Set();
    }
    ...
    <div className="h-full w-full flex flex-col bg-background overflow-hidden">
      ...
      <div className={cn(
        ...
        <div 
          ...
          onTouchEnd={() => {
            if (sheetOffset > 80) { navigator.vibrate?.(10); setMenuOpen(false); }
            setSheetOffset(0);
          }}
        >
          ...
          <div className="flex rounded-xl border border-border overflow-hidden text-sm font-medium mb-4">
            <button onClick={() => handleViewModeChange('all')} className={cn('flex-1 py-2.5 text-center transition-colors', viewMode === 'all' ? 'bg-cyan-500/10 text-cyan-500 font-semibold' : 'hover:bg-muted')}>All</button>
            <button onClick={() => handleViewModeChange('spots')} className={cn('flex-1 py-2.5 text-center border-x border-border transition-colors', viewMode === 'spots' ? 'bg-cyan-500/10 text-cyan-500 font-semibold' : 'hover:bg-muted')}>🏄 Spots</button>
            <button onClick={() => handleViewModeChange('amenities')} className={cn('flex-1 py-2.5 text-center transition-colors', viewMode === 'amenities' ? 'bg-cyan-500/10 text-cyan-500 font-semibold' : 'hover:bg-muted')}>🗺️ Amenities</button>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                onClick={() => setSelectedTypes(selectedTypes.size === 0 ? new Set(amenityTypes) : new Set())}
                className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-colors', selectedTypes.size === 0 ? 'bg-primary/10 border-primary/30 text-primary' : 'border-border hover:bg-muted')}
              >All/None</button>
              {amenityTypes.map((type) => {
                const isActive = selectedTypes.has(type);
                return (
                  <button key={type} onClick={() => { const n = new Set(selectedTypes); n.has(type) ? n.delete(type) : n.add(type); setSelectedTypes(n); }}
                    className={cn('px-3 py-1.5 rounded-full text-xs font-medium border transition-colors flex items-center gap-1', isActive ? 'bg-primary/10 border-primary/30 text-primary' : 'border-border hover:bg-muted')}
                  >
          ...
          <div className="mt-5 flex gap-3">
            <div className="flex-1 rounded-xl bg-gradient-to-br from-pink-500 to-purple-600 p-[1.5px]">
              <button onClick={() => { setSearchQuery(''); setViewMode('all'); setSelectedTypes(new Set()); }}
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        {(selectedSpot || selectedAmenity) && (
          <div className="fixed inset-0 top-0 z-[1050] bg-black/20 backdrop-blur-[1px]" onClick={handleClearSelection} />
        )}
        ...
        <aside className={cn(
          ...
          {selectedSpot && (
            <div>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {selectedSpot.beachName && <p className="text-xs text-muted-foreground">{selectedSpot.beachName}</p>}
                    </div>
                  </div>
                  <button onClick={handleClearSelection} className="p-1.5 rounded-xl hover:bg-muted transition-colors group">
                    <X className="h-4 w-4 text-muted-foreground group-hover:text-foreground transition-colors" />
                  </button>
                </div>

                <div className="flex flex-wrap gap-1.5 mb-4">
                  <span className={cn('px-2.5 py-0.5 rounded-full text-xs font-medium border', difficultyColors[selectedSpot.difficulty])}>{selectedSpot.difficulty}</span>
                  <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gradient-to-br from-teal-400/10 to-cyan-500/10 text-cyan-700 dark:text-cyan-300 border border-teal-200 dark:border-teal-800">{selectedSpot.waveType.replace('-', ' ')}</span>
                  {selectedSpot.location.region && (
                    <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 border border-gray-200 dark:border-gray-700">{selectedSpot.location.region}</span>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-2.5 text-sm mb-4">
                  ...
                  <div className="p-3 rounded-xl bg-muted/70 col-span-2">
                    ...
                    <span className="font-semibold font-mono text-xs">{selectedSpot.location.lat.toFixed(4)}, {selectedSpot.location.lon.toFixed(4)}</span>
                  </div>
                </div>
              </div>
                ...
                <div className="mb-2">
                  ...
                  <div className="space-y-2">
                    {spotAmenities.map((amenity) => (
                      <button key={amenity.id} onClick={() => handleAmenitySelect(amenity)}
              ...
              {spotAmenities.length === 0 && (
                <p className="text-xs text-muted-foreground italic mt-2 mb-2">No amenity data yet for this spot.</p>
              )}
            </div>
          )}
          ...
          {selectedAmenity && !selectedSpot && (
            <div>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center gap-3">
                    <div className={cn('w-10 h-10 rounded-xl flex items-center justify-center text-lg shadow-md', amenityColors[selectedAmenity.type])}>
                ...
                <div className="flex items-center gap-2 mb-4">
                  <span className={cn('px-2.5 py-0.5 rounded-full text-xs font-medium', amenityColors[selectedAmenity.type])}>
                ...
                {parentSpot && (
                  <button onClick={() => handleSpotSelect(parentSpot)}
                ...
                <div className="p-3 rounded-xl bg-muted/70">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground block mb-0.5">Coordinates</span>
                  <span className="font-semibold font-mono text-xs">{selectedAmenity.location.lat.toFixed(4)}, {selectedAmenity.location.lon.toFixed(4)}</span>
                </div>
              </div>
            </div>
          )}
          ...
          {!selectedSpot && !selectedAmenity && (
            <div className="p-4">
              <div className="flex border-b border-border mb-4">
                <button onClick={() => setSidebarTab('spots')} className={cn('px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px', sidebarTab === 'spots' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-muted-foreground hover:text-foreground')}>
                  🏄 Spots ({filteredSpots.length})
                </button>
                <button onClick={() => setSidebarTab('amenities')} className={cn('px-4 py-2 text-sm font-medium border-b-2 transition-colors -mb-px', sidebarTab === 'amenities' ? 'border-cyan-400 text-cyan-400' : 'border-transparent text-muted-foreground hover:text-foreground')}>
              ...
              {sidebarTab === 'spots' && (
                <div className="space-y-2">
                  {filteredSpots.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No spots match your search</p>}
                  {filteredSpots.map((spot) => (
                    <button key={spot.id} onClick={() => handleSpotSelect(spot)}
              ...
              {sidebarTab === 'amenities' && (
                <div className="space-y-2">
                  {filteredAmenities.length === 0 && <p className="text-sm text-muted-foreground text-center py-8">No amenities match your search</p>}
